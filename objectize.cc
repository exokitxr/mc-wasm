...
EMSCRIPTEN_KEEPALIVE void doCollide(float *positions, unsigned int numPositions, float origin[3], float direction[3], unsigned int range, float *collision, float *rangeIndexFlags, unsigned int *rangePositionIndex) {
  collide(positions, numPositions, origin, direction, range, collision, rangeIndexFlags, *rangePositionIndex);
}
