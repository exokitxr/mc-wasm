...
EMSCRIPTEN_KEEPALIVE void doSmoothedPotentials(int *chunkCoords, float *colorTargetCoordBuf, unsigned int numChunkCoords, unsigned int numColorTargetCoords, int width, int height, int depth, int colorTargetSize, float voxelSize, float *potentialsBuffer) {
  smoothedPotentials(chunkCoords, colorTargetCoordBuf, numChunkCoords, numColorTargetCoords, width, height, depth, colorTargetSize, voxelSize, potentialsBuffer);
}
