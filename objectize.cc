...
EMSCRIPTEN_KEEPALIVE void doSmoothedPotentials(int *chunkCoords, float *colorTargetCoordBuf, unsigned int numChunkCoords, unsigned int numColorTargetCoords, int width, int height, int depth, int colorTargetSize, float voxelSize, unsigned int *numPotentialsPointers, float *potentialsPointers) {
  std::vector<std::vector<float>> potentialsArray = smoothedPotentials(chunkCoords, colorTargetCoordBuf, numChunkCoords, numColorTargetCoords, width, height, depth, colorTargetSize, voxelSize);
}
